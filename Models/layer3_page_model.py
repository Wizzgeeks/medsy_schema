...
class Layer3_page(Document):
    ...
    types = StringField(choices=['content','mcq','test_series','exam','adaptive_learning','adaptive_learning_mcq','adaptive_learning_content',"related_videos","active_recall","active_recall_mcq","active_recall_content","crossword_puzzle","crossword_dragger"],required=True)
    sequence = IntField(required=True)
    hierarcy_level = IntField(default=0)
    enable_days = IntField(default=0)
    child_pages = ListField(ReferenceField("Layer3_page", reverse_delete_rule=CASCADE, required=True))
